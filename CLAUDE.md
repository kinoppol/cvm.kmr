...
### Public landing page
`/` serves `LandingController` — a guest-facing course showcase (`resources/views/landing.latte`, its own `public/assets/css/landing.css`, originally styled after the Colorlib "Education" template kept in `resources/design/course-template/`, gitignored, now redesigned with an AI-era look under `--lp-*` CSS variables independent of the app theme). Logged-in users are redirected to `/dashboard`. A teacher opts their courses in via the switch on `/courses` (`POST /courses/landing-visibility` → `courses.show_on_landing`, default 0 for all).
